...
public class LifeGame extends JFrame implements ActionListener {
    ...
    public void initGUI() {
        ...
        map = new GameMap(maxWidth, maxLength);

        JPanel backPanel, centerPanel, bottomPanel;
        JLabel jWidth, jLength, jNowGeneration,jTime;
        ...
        jTime = new JLabel(" 时间间隔：");
        jbNowGeneration = new JButton(""+map.getNowGeneration());//Buttom不能直接添加int，故采用此方式
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == ok) { //确定
            ...
            map = new GameMap(getMaxWidth(), getMaxLength());

        } else if(e.getSource() == clearGeneration){ //代数清零
            map.setNowGeneration(0);
            jbNowGeneration.setText(""+map.getNowGeneration());//刷新当前代数
